...
def barGen(barCount, bg_color, bd_color):
    barCSS = f".container {{ background-color: #{bg_color}; border: 2px solid #{bd_color}; }}\n"
    barCSS += f".spotify-logo svg path {{ fill: #{bd_color}; }}\n"
    left = 1
...
def makeSVG(track, bg_color, bd_color):
    barCount = 84
    contentBar = "".join(["<div class='bar'></div>" for i in range(barCount)])
    barCSS = barGen(barCount, bg_color, bd_color)

    if not track:
        artistName = "Configure Last.fm variables"
        songName = "No track found"
        songURI = "#"
        artistURI = "#"
        ...
        artistName = track.get("artist", {}).get("#text", "Unknown Artist").replace("&", "&amp;")
        songName = track.get("name", "Unknown Track").replace("&", "&amp;")
        songURI = track.get("url", "#")
        
        artist_url_encoded = urllib.parse.quote(track.get("artist", {}).get("#text", ""))
        artistURI = f"https://www.last.fm/music/{artist_url_encoded}" if artist_url_encoded else "#"

        is_playing = track.get("@attr", {}).get("nowplaying") == "true"
        currentStatus = "Vibing to:" if is_playing else "Was playing:"
        ...
        image_url = ""
        for img in images:
            if img.get("size") in ["large", "extralarge"]:
...
def catch_all(path):
    bg_color = request.args.get('background_color') or "181414"
    bd_color = request.args.get('border_color') or "181414"
    bg_color = bg_color.lstrip('#')
    bd_color = bd_color.lstrip('#')
    track = get_lastfm_track()
    svg = makeSVG(track, bg_color, bd_color)
